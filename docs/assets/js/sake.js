/*
 * ==========================================================================================
 *  Sake Documentation JavaScript
 *  File: docs/assets/js/sake.js
 *
 *  Purpose:
 *      Provides safe progressive enhancements for the Sake MkDocs Material documentation site.
...
( function()
{
	...
	const SakeDocs = {
		config: {
			scrollTopId: "sake-scroll-top",
			pageToolsId: "sake-page-tools",
			progressId: "sake-reading-progress",
			tableFilterClass: "sake-table-filter",
			headingLinkClass: "sake-heading-link",
			codeLabelClass: "sake-code-label",
			codeToggleClass: "sake-code-toggle",
			initializedAttribute: "data-sake-enhanced",
			navScrollKey: "sake-docs-nav-scroll",
			...
			tableSelector: ".md-typeset table:not([data-sake-no-filter])",
		...
		handleDocumentClick: function( event )
		{
			...
			if( target.closest && target.closest( "[data-sake-copy-heading]" ) )
			{
				event.preventDefault();
				this.copyHeadingLink( target.closest( "[data-sake-copy-heading]" ) );
				return;
			}
			if( target.closest && target.closest( "[data-sake-copy-page]" ) )
			{
				event.preventDefault();
				this.copyPageLink( target.closest( "[data-sake-copy-page]" ) );
				return;
			}
			if( target.closest && target.closest( "[data-sake-print-page]" ) )
			{
				event.preventDefault();
				window.print();
				return;
			}
			if( target.closest && target.closest( "[data-sake-toggle-code]" ) )
			{
				event.preventDefault();
				this.toggleCodeBlock( target.closest( "[data-sake-toggle-code]" ) );
				return;
			}
			if( target.closest && target.closest( "[data-sake-api-expand]" ) )
			{
				event.preventDefault();
				this.setApiDetailsState( true );
				return;
			}
			if( target.closest && target.closest( "[data-sake-api-collapse]" ) )
			{
				event.preventDefault();
				this.setApiDetailsState( false );
				return;
			}
			if( target.closest && target.closest( "[data-sake-api-clear]" ) )
			{
				event.preventDefault();
				this.clearApiFilter();
			}
		...
		handleKeyboardShortcuts: function( event )
		{
			...
			{
				const apiSearch = document.getElementById( "sake-api-search" );
				if( apiSearch )
				{
					event.preventDefault();
					apiSearch.focus();
			...
			links.forEach( function( link )
			{
				try
				{
					...
					if( url.host && url.host !== currentHost )
					{
						...
						link.classList.add( "sake-external-link" );
						if( !link.querySelector( ".sake-external-indicator" ) )
						{
							const indicator = document.createElement( "span" );
							indicator.className = "sake-external-indicator";
		...
		customizeSearch: function()
		{
			...
			{
				input.setAttribute( "placeholder", "Search Sake docs..." );
				input.setAttribute( "aria-label", "Search Sake documentation" );
			} );
		},
		...
		addScrollTopButton: function()
		{
			...
			button.type = "button";
			button.className = "sake-scroll-top";
		...
		addPageTools: function()
		{
			...
			tools.className = "sake-page-tools";
			tools.innerHTML = [
				"<button type=\"button\" data-sake-copy-page title=\"Copy page link\" aria-label=\"Copy page link\">Copy link</button>",
				"<button type=\"button\" data-sake-print-page title=\"Print page\" aria-label=\"Print page\">Print</button>"
			].join( "" );
		...
		{
			const button = document.querySelector( "[data-sake-copy-page]" );
			this.copyTextToClipboard( window.location.href, button, "Copied", "Copy link" );
		},
		addHeadingLinks: function()
		{
			...
			{
				if( heading.querySelector( "." + SakeDocs.config.headingLinkClass ) )
				{
					return;
				}
				const button = document.createElement( "button" );
				button.type = "button";
				button.className = SakeDocs.config.headingLinkClass;
				button.setAttribute( "data-sake-copy-heading", heading.id );
		...
		{
			const id = button.getAttribute( "data-sake-copy-heading" );
			if( !id )
			{
				return;
			}
		...
		copyTextToClipboard: function( text, button, successText, defaultText )
		{
			...
			if( navigator.clipboard && typeof navigator.clipboard.writeText === "function" )
			{
				...
				{
					SakeDocs.fallbackCopyText( text );
					updateButton();
				} );
				return;
			}
		...
		addTableFilters: function()
		{
			...
			{
				if( table.getAttribute( "data-sake-filtered" ) === "true" )
				{
					return;
				}
				...
				if( rows.length < SakeDocs.config.largeTableMinimumRows )
				{
					return;
				}
				table.setAttribute( "data-sake-filtered", "true" );
				const wrapper = document.createElement( "div" );
				wrapper.className = "sake-table-tools";
				const input = document.createElement( "input" );
				input.type = "search";
				input.className = SakeDocs.config.tableFilterClass;
				input.placeholder = "Filter table...";
				input.setAttribute( "aria-label", "Filter table " + ( index + 1 ) );
				const count = document.createElement( "span" );
				count.className = "sake-table-count";
				...
				{
					SakeDocs.filterTable( table, input.value, count );
				} );
			} );
		},
		...
		addCodeLabels: function()
		{
			...
			codeBlocks.forEach( function( code )
			{
				...
				if( !pre || pre.getAttribute( "data-sake-labeled" ) === "true" )
				{
					return;
				}
				const language = SakeDocs.detectCodeLanguage( code );
				if( !language )
				{
					return;
				}
				pre.setAttribute( "data-sake-labeled", "true" );
				const label = document.createElement( "div" );
				label.className = SakeDocs.config.codeLabelClass;
		...
		addCodeToggles: function()
		{
			...
			{
				const pre = code.parentElement;
				if( !pre || pre.getAttribute( "data-sake-toggle-ready" ) === "true" )
				{
					return;
				}
				pre.setAttribute( "data-sake-toggle-ready", "true" );
				if( pre.scrollHeight <= SakeDocs.config.maxCollapsedCodeHeight + 80 )
				{
					return;
				}
				pre.classList.add( "sake-code-collapsed" );
				pre.style.maxHeight = SakeDocs.config.maxCollapsedCodeHeight + "px";
				const button = document.createElement( "button" );
				button.type = "button";
				button.className = SakeDocs.config.codeToggleClass;
				button.setAttribute( "data-sake-toggle-code", "collapsed" );
		...
		toggleCodeBlock: function( button )
		{
			...
			const state = button.getAttribute( "data-sake-toggle-code" );
			if( state === "collapsed" )
			{
				pre.classList.remove( "sake-code-collapsed" );
				pre.style.maxHeight = "";
				button.setAttribute( "data-sake-toggle-code", "expanded" );
		...
		enhanceKeyboardFocus: function()
		{
			document.body.addEventListener( "keydown", function( event )
			{
				if( event.key === "Tab" )
				{
					document.body.classList.add( "sake-keyboard-mode" );
				}
			} );
			document.body.addEventListener( "mousedown", function()
			{
				document.body.classList.remove( "sake-keyboard-mode" );
			} );
		},
		enhanceApiReference: function()
		{
			...
			{
				if( container.getAttribute( "data-sake-api-enhanced" ) === "true" )
				{
					return;
				}
				container.setAttribute( "data-sake-api-enhanced", "true" );
				const heading = container.querySelector( "h2, h3, h4, h5" );
				if( !heading || heading.querySelector( ".sake-api-badge" ) )
				{
					return;
				}
				const badge = document.createElement( "span" );
				badge.className = "sake-api-badge";
		...
		{
			const content = document.querySelector( this.config.contentSelector );
			if( !content || content.querySelector( ".sake-api-tools" ) )
			{
				return;
			}
			...
			panel.className = "sake-api-tools";
			panel.setAttribute( "aria-label", "API tools" );
			panel.innerHTML = [
				"<h2 class=\"sake-api-tools-title\">API Tools</h2>",
				"<label class=\"sake-api-search-label\" for=\"sake-api-search\">Filter classes, methods, properties, or text</label>",
				"<input id=\"sake-api-search\" class=\"sake-api-search\" type=\"search\" placeholder=\"Filter classes, methods, properties, or text...\" autocomplete=\"off\">",
				"<div class=\"sake-api-tool-buttons\">",
				"<button type=\"button\" class=\"sake-api-tool-button\" data-sake-api-expand>Expand all</button>",
				"<button type=\"button\" class=\"sake-api-tool-button\" data-sake-api-collapse>Collapse all</button>",
				"<button type=\"button\" class=\"sake-api-tool-button\" data-sake-api-clear>Clear filter</button>",
				"</div>",
				"<p class=\"sake-api-filter-status\" aria-live=\"polite\"></p>"
			].join( "" );
			firstHeading.insertAdjacentElement( "afterend", panel );
			const input = panel.querySelector( "#sake-api-search" );
			const status = panel.querySelector( ".sake-api-filter-status" );
			if( input )
			{
				input.addEventListener( "input", function()
				{
					SakeDocs.filterApiObjects( input.value, status );
				} );
			}
		},
		filterApiObjects: function( query, statusElement )
		{
			...
			objects.forEach( function( object )
			{
				...
				if( !normalizedQuery || text.indexOf( normalizedQuery ) !== -1 )
				{
					object.classList.remove( "sake-api-hidden" );
					visibleCount += 1;
				}
				else
				{
					object.classList.add( "sake-api-hidden" );
				}
			} );
			if( statusElement )
			{
		...
		{
			const input = document.getElementById( "sake-api-search" );
			const status = document.querySelector( ".sake-api-filter-status" );
			if( !input )
			{
				return;
			}
		...
		enhanceTocProgress: function()
		{
			...
			if( !toc || toc.getAttribute( "data-sake-toc-enhanced" ) === "true" )
			{
				return;
			}
			toc.setAttribute( "data-sake-toc-enhanced", "true" );
		},
		updateTocProgress: function()
		{
			...
			tocLinks.forEach( function( link )
			{
				...
				if( href === activeId )
				{
					link.classList.add( "sake-toc-active" );
				}
				else
				{
					link.classList.remove( "sake-toc-active" );
				}
			} );
		}
	};
